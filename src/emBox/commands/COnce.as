package emBox.commands {
	...
	public class COnce extends ACommand {
		...
		override protected function doIn():void {
			super.doIn();
			if (!isInExecuted){
				command.addEventListener(CEvent.EXECUTE_IN_COMPLETE, onComplete);
				command.executeIn();
			}else																executeInComplete();
		}
		private function onComplete(e:Event):void {
			command.removeEventListener(CEvent.EXECUTE_IN_COMPLETE, onComplete);
			isInExecuted = true;
			executeInComplete();
		}
		...
		override protected function doOut():void {
			super.doOut();
			if (!isOutExecuted){
				command.addEventListener(CEvent.EXECUTE_OUT_COMPLETE, onCompleteOut);
				command.executeOut();
			}else																executeOutComplete();
		}
		
		private function onCompleteOut(e:Event):void {
			command.removeEventListener(CEvent.EXECUTE_OUT_COMPLETE, onCompleteOut);
			isOutExecuted = true;
			executeOutComplete();
		}
