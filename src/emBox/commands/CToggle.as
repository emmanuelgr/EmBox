package emBox.commands {
	...
	public class CToggle extends ACommand implements IToggle{
		...
		override protected function doIn():void  {
			super.doIn();
			if (_toggleState){
				cmndA.addEventListener(CEvent.EXECUTE_IN_COMPLETE, subExecuteInComplete);
				cmndA.executeIn(eventIn);
			} else {
				cmndB.addEventListener(CEvent.EXECUTE_IN_COMPLETE, subExecuteInComplete);
				cmndB.executeIn(eventIn);
			}
		}
		private function subExecuteInComplete(e:Event):void {
			(e.target as ACommand).removeEventListener(CEvent.EXECUTE_IN_COMPLETE, subExecuteInComplete);
			executeInComplete();
		}
		...
		override protected function doOut():void {
			super.doOut();
			if (_toggleState){
				cmndA.addEventListener(CEvent.EXECUTE_OUT_COMPLETE, subExecuteOutComplete);
				cmndA.executeOut(eventOut);
			} else {
				cmndB.addEventListener(CEvent.EXECUTE_OUT_COMPLETE, subExecuteOutComplete);
				cmndB.executeOut(eventOut);
			}
		}
		private function subExecuteOutComplete(e:Event):void {
			(e.target as ACommand).removeEventListener(CEvent.EXECUTE_OUT_COMPLETE, subExecuteOutComplete);
			executeOutComplete();
		}
		...
		public function toggle( e:Event = null ):void {
			...
			if (parallelSwap){
				...
				if (_toggleState){
					cmndB.addEventListener( CEvent.EXECUTE_OUT_COMPLETE, onCurrentCmndOut);
					cmndB.executeOut();
				}else {
					cmndA.addEventListener( CEvent.EXECUTE_OUT_COMPLETE, onCurrentCmndOut);
					cmndA.executeOut();
				}
			}
		}
		private function onCurrentCmndOut(e:CEvent):void {
			e.target.removeEventListener( CEvent.EXECUTE_OUT_COMPLETE, onCurrentCmndOut);
