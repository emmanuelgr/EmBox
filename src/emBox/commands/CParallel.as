...
public class CParallel extends ACommand {
  ...
  override protected function doIn():void {
    ...
    for (var i:int = 0; i < cmnds.length; i++){
      ...
      aCmnd.addEventListener(CEvent.EXECUTE_IN_COMPLETE, onSubcommandComplete);
      aCmnd.executeIn();
    }
  }
  private function onSubcommandComplete(e:Event=null):void {
    ACommand(e.currentTarget).removeEventListener(CEvent.EXECUTE_IN_COMPLETE, onSubcommandComplete);
    completeInCount++;
  ...
  override protected function doOut():void {
    ...
    for (var i:int = cmnds.length-1; i >=0 ; i--){
      ...
      aCmnd.repeatable = repeatable;
      aCmnd.cancelable = cancelable;
      aCmnd.addEventListener(CEvent.EXECUTE_OUT_COMPLETE, onSubcommandOutComplete);
      aCmnd.executeOut();
    }
  }
  private function onSubcommandOutComplete(e:Event=null):void {
    ACommand(e.target).removeEventListener(CEvent.EXECUTE_OUT_COMPLETE, onSubcommandOutComplete);
    completeOutCount++;
