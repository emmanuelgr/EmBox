package emBox.shapes {
  import emBox.EmBox;
	 ...
	 * @author Emmanuel
	 */
	public class Star extends EmBoxShape {
		...
		override protected function draw():void {
			graphics.clear();
			var point:Point;
			var stepRadian:Number = Maths.twoPi / (_rays *2);
			...
			for (var i:int = 0; i < _rays*2 ; i+=2) {
				point = Point.polar( _radiusInner, (stepRadian * (i + 0)) + Maths.halfPi );
				graphics.lineTo( point.x + centerX, point.y + centerY );
				point = Point.polar( _radius,  (stepRadian * (i + 1)) + Maths.halfPi );
				graphics.lineTo( point.x + centerX, point.y + centerY );
			}
			point = Point.polar( _radiusInner, (stepRadian * _rays*2) + Maths.halfPi );
			graphics.lineTo( point.x + centerX, point.y + centerY );
			graphics.endFill();
		}
