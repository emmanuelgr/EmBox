package emBox.utils{
    ...
    // TODO Check if need to wrap twice for negative
      ...
      var result:Number = w(valueToWrap);
			if (!result) 	result = 0;	//so it wont return NaN
			if (result<0) 	result = w(result);	//so it wont return negative
      //trace("valueToWrap: " + valueToWrap + " result: " + result)
		...
		public static function ezCirc (v:Number, p:Number) :Number{
			v = -(Math.sqrt(1-v*v)-1);
			if(p>1) return easeIn(v, --p, ezCirc);
			return v;
		}
    ...
    //package emBoxAS3Lib.com.flashdynamix.motion.plugins
