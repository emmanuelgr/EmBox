...
public class CSWFAddress extends ACommand {
	public static const SITESFRONT:String="sitesfront";
  ...
  private var voList:ACollectionDispatcher;
	 ...
	 * @param	forceToURL usually "/home" if "sitesfront" will get drupals 7 site front page
	 */
  ...
  forceToURL:String = null
    ...
    executeInComplete();
    if (forceToURL == SITESFRONT) {
      Main.voList.setPointerByPropertyWithValue("nid", EmBoxD7.sitesFront.substr(5));
    }else if (forceToURL!="") {
  ...
  private function onModelPointer(e:Event):void {
    ...
    aVO = voList.pointedObj as VODrupalNode;
    if (!aVO && "" == requestedSWFAdddressPath) return;
    ...
    var swfAddReqVO:VODrupalNode = voList.objectByPropertyWithValue("path", requestedSWFAdddressPath) as VODrupalNode;
    trace(">>>>>>>>swfAddReqVO: " + swfAddReqVO);
    /// Update voList model
    var swfAddReqVOIndex:int = voList.list.indexOf(swfAddReqVO);
    trace(">>>>>>>>swfAddReqVOIndex: " + swfAddReqVOIndex);
